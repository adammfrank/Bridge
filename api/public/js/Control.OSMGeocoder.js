...
L.Control.OSMGeocoder = L.Control.extend({
	...
	onAdd: function (map) {
		...
		$(form).find('input').first().css({'margin-top': '0', 'border-color': '#26ade4', 'background': 'rgba(255, 255, 255, .8)'}).attr('placeholder', 'Location');
		$(form).find('input').last().css({'margin-top': '8px', 'display': 'none'});
		$(form).css({'margin-top': '-14px', 'margin-right': '8px'});
		container.appendChild(form);
		$(container).css({margin: 0});
		$(form).find('input').first().keyup(function(e){
		    if(e.keyCode == 13)
		    {
		        $(form).find('input').last().click();
		    }
		});
		return container;
	},
